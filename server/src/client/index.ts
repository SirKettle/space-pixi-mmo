...
export const handleDisconnect = (
  io: TIOServer, client: IClient
) => () => {
  console.log(`user disconnected: ${client.user.username}`)
  // Remove the current client
  serverState.clients = serverState.clients.filter(s => s.user.username !== client.user.username);

  // broadcast to all other users
  client.socket.broadcast.emit('playerLeft', client.user);
  // broadcast to all users
  io.emit('activeUsersUpdate', serverState.clients.map(s => s.user));
...
export const handleToggleDebug = (
  io: TIOServer, client: IClient
) => () => {

}

export const handlePauseGame = (
  io: TIOServer, client: IClient
) => () => {
...
(socket: TClient) => {
  ...
  const client: IClient = {
    ...
    user: {
      username: generateUsername(Object.values(serverState.clients))(socket.data.username)
    }
  }
  ...
  socket.on('pingTest', (t) => { socket.emit('pingTestResponse', t); });
